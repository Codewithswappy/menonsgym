  ...
  return (
    ...
    >
      ...
      <div className="bg-white/95 backdrop-blur-md border border-slate-200/90 text-slate-900 text-[9px] uppercase font-bold tracking-wider px-2.5 py-1.5 rounded-full shadow-md hidden md:flex items-center gap-1.5 select-none">
        <div className="w-2 h-2 animate-pulse rounded-full bg-amber-500 stroke-none shrink-0" />
        <span>Chat on WhatsApp</span>
      </div>

      {/* Yellow Circular Floating Action Bubble */}
      <a
        href={whatsappUrl}
        target="_blank"
        rel="noopener noreferrer"
        className="relative w-12 h-12 sm:w-14 sm:h-14 bg-yellow-400 hover:bg-yellow-300 text-slate-950 rounded-full flex items-center justify-center shadow-xl shadow-yellow-500/25 hover:scale-110 active:scale-95 transition-all group"
      ...
      >
        ...
        <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 sm:w-4 sm:h-4 bg-amber-500 border-2  border-white rounded-full" />
      </a>
    </motion.div>
  );
}
