...
const TIMING_OPTIONS = [
  'Morning (5:30 AM - 11 AM)',
  'Afternoon (11 AM - 4 PM)',
  'Evening (4 PM - 10:30 PM)',
];

export default function Contact({ onOpenBooking }: ContactProps) {
  const [formName, setFormName] = useState('');
  const [formPhone, setFormPhone] = useState('');
  const [formTiming, setFormTiming] = useState('Morning (5:30 AM - 11 AM)');
  ...
  const handleQuickInquiry = (e: React.FormEvent) => {
    ...
    const message = `Hello MENON'S GYM!%0A%0AName: ${encodeURIComponent(formName)}%0APhone: ${encodeURIComponent(formPhone)}%0APreferred Timing: ${encodeURIComponent(formTiming)}%0A%0AI would like to book a free walk-in trial pass at your Greater Khanda location!`;
    ...
    <section id="contact" className="py-20 sm:py-28 bg-white relative">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="max-w-xl space-y-2 mb-12">
          <span className="text-xs font-bold uppercase tracking-widest text-yellow-700 bg-yellow-100 px-3 py-1 rounded-full border border-yellow-200 inline-block">
            Visit & Contact
          </span>
          ...
          <div className="lg:col-span-6 space-y-6 flex flex-col justify-between">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <a
                      ...
                      className="text-slate-800 hover:text-yellow-700 font-bold mt-0.5 inline-block"
                    >
                      {GYM_DETAILS.phone}
                    </a>
                  </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <a
                      ...
                      className="text-yellow-700 hover:underline font-bold mt-0.5 inline-block"
                    >
                      Chat on WhatsApp →
                    </a>
                  </div>
            ...
            <div className="p-6 rounded-3xl bg-slate-50 border border-slate-200/90 space-y-4 shadow-xs">
              ...
              <form onSubmit={handleQuickInquiry} className="space-y-3.5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-slate-200/90 rounded-xl pl-10 pr-3.5 py-3 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-yellow-500 shadow-2xs"
                    />
                  </div>
                ...
                {/* Custom Ultra-Clean Animated Dropdown */}
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                    className="w-full bg-white border border-slate-200/90 rounded-xl px-4 py-3 text-xs text-slate-900 flex items-center justify-between font-semibold focus:outline-none focus:border-yellow-500 shadow-2xs cursor-pointer select-none"
                  >
                    <span>{formTiming}</span>
                    <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isDropdownOpen ? 'rotate-180 text-yellow-600' : ''}`} />
                  </button>
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left px-3.5 py-2.5 rounded-lg text-xs font-semibold flex items-center justify-between transition-colors cursor-pointer ${
                                isSelected ? 'bg-yellow-50 text-slate-950 font-bold' : 'hover:bg-slate-50 text-slate-700'
                              }`}
                            >
                              <span>{option}</span>
                              {isSelected && <Check className="w-3.5 h-3.5 text-yellow-600" />}
                            </button>
                          );
                        })}
                      </motion.div>
                ...
                <button
                  type="submit"
                  disabled={isSent}
                  className="w-full py-3.5 px-4 bg-yellow-400 hover:bg-yellow-300 text-slate-950 font-black rounded-xl text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer shadow-xs hover:shadow-md"
                >
          ...
          <div className="lg:col-span-6 rounded-md overflow-hidden border border-slate-200 bg-slate-100 min-h-[440px] relative shadow-xs flex flex-col">
            <div className="p-3 bg-white border-b border-slate-200/80 flex items-center justify-between text-xs font-bold text-slate-900">
              <span className="flex items-center gap-1.5">
                <MapPin className="w-4 h-4 text-yellow-600" />
                <span>MENON'S GYM — Greater Khanda, Panvel</span>
              </span>
              <a
                href="https://maps.google.com/?q=MENON'S+GYM+Greater+Khanda+Panvel"
                target="_blank"
                rel="noopener noreferrer"
                className="text-yellow-700 hover:underline inline-flex items-center gap-1 text-[11px]"
              >
            ...
            <iframe
              title="MENON'S GYM Location Map Pinpoint"
              src="https://maps.google.com/maps?q=MENON'S+GYM+Greater+Khanda+Panvel+Maharashtra&t=&z=16&ie=UTF8&iwloc=&output=embed"
              width="100%"
              height="100%"
              style={{ border: 0 }}
