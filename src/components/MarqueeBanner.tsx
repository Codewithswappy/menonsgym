          ...
          >
            {[...items, ...items, ...items, ...items].map((text, idx) => (
              <div key={idx} className="flex items-center gap-6 text-xs sm:text-sm font-black uppercase tracking-widest">
                <span>{text}</span>
                <Asterisk className="w-4 h-4 text-yellow-400 shrink-0" />
              </div>
            ))}
          </motion.div>
        </div>

        {/* Ribbon 2: Angled Yellow Ribbon (rotate-2 crossing over) */}
        <div className="w-[120%] rotate-2 bg-yellow-400 text-slate-950 py-3 shadow-md z-20 overflow-hidden flex whitespace-nowrap -mt-6">
          <motion.div
            animate={{ x: [-1000, 0] }}
