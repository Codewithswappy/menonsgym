...
export default function BookingModal({ isOpen, onClose, defaultPlan = 'Free Trial Pass' }: BookingModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const message = `Hello MENON'S GYM!%0A%0AMy Name: ${encodeURIComponent(name)}%0AMy Phone: ${encodeURIComponent(phone)}%0AMy Goal: ${encodeURIComponent(selectedGoal)}%0ARequested Pass: ${encodeURIComponent(defaultPlan)}%0A%0AI would like to claim my free trial pass and visit your Greater Khanda location!`;
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          >
            ...
            {/* Modal Header */}
            <div className="mb-4">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-yellow-100 border border-yellow-200/80 text-yellow-700 rounded-full text-[11px] font-extrabold uppercase tracking-wider mb-2">
            ...
            {isSubmitted ? (
              <div className="py-8 text-center space-y-3">
                <motion.div
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="w-14 h-14 bg-yellow-100 border border-yellow-200 text-yellow-700 rounded-full flex items-center justify-center mx-auto"
                >
                  <CheckCircle2 className="w-8 h-8" />
                </motion.div>
                <h4 className="text-base font-extrabold text-slate-900">Redirecting to WhatsApp...</h4>
                <p className="text-xs text-slate-500 max-w-xs mx-auto font-medium">
                  Connecting you directly with our manager at MENON'S GYM.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-3">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-3.5 py-2.5 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-yellow-500 focus:bg-white text-xs font-medium transition-all shadow-2xs"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 flex items-center justify-between font-medium focus:outline-none focus:border-yellow-500 shadow-2xs cursor-pointer select-none"
                    >
                ...
                <div className="pt-1.5 space-y-2">
                  <button
                    type="submit"
                    className="w-full py-3 px-4 bg-yellow-400 hover:bg-yellow-300 text-slate-950 font-black rounded-xl shadow-xs hover:shadow-md transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wider cursor-pointer"
                  >
                  ...
                  >
                    <PhoneCall className="w-3.5 h-3.5 text-yellow-700" />
