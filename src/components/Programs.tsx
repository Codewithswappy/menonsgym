          ...
          {PROGRAMS.map((prog, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`${spanClass} relative rounded-md overflow-hidden bg-slate-900 border border-slate-200/80 hover:border-yellow-500/70 cursor-pointer group shadow-xs hover:shadow-md transition-all duration-300 min-h-[280px] sm:h-72 select-none flex flex-col justify-between p-6`}
              >
                {/* Photo Background */}
                <Image
                  src={prog.image}
                ...
                <div className="relative z-10 flex items-center justify-between">
                  <div className="w-9 h-9 rounded-xl bg-yellow-400 text-slate-950 flex items-center justify-center shadow-xs shrink-0">
                ...
                <div className="relative z-10 text-white space-y-2 max-w-xl">
                  <h3 className="text-xl sm:text-2xl font-black uppercase tracking-wide leading-tight text-white group-hover:text-yellow-400 transition-colors">
                    {prog.title}
                  </h3>
