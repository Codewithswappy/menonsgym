...
export default function Testimonials() {
  ...
  const renderCard = (review: (typeof TESTIMONIALS)[0], idx: number) => (
    <div
      key={`${review.id}-${idx}`}
      className="w-64 sm:w-72 shrink-0 bg-white border border-slate-200/90 hover:border-yellow-500/60 rounded-2xl p-4 shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)] hover:shadow-md transition-all duration-300 flex flex-col justify-between space-y-3.5 select-none"
    >
      ...
      <div className="space-y-2.5">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-extrabold text-yellow-700 bg-yellow-100/90 px-2 py-0.5 rounded-full border border-yellow-200/80 uppercase tracking-wider">
            Verified Review
          </span>
        </div>
      ...
      <div className="flex items-center gap-2.5 pt-2.5 border-t border-slate-100">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-xs font-extrabold text-slate-900 flex items-center gap-1 truncate">
            ...
            <CheckCircle2 className="w-3 h-3 text-yellow-600 shrink-0" />
          </div>
